	...
	if (process.argv[3] && parseInt(process.argv[3]) !== -1) {
		...
		await Promise.all(
			oldSubnetEvents.map(async (event) => {
				...
				if (!isProcessed) {
					/* If not processed add tx to txs array */
					console.log("OLD: is not processed, will release on avax\n");
					txs.push(event);
				} else {
					console.log("OLD: is already processed\n");
				}
	...
	bridgeContracts.avax.admin.on(
		...
		async (from, to, amount, date, nonce, type) => {
			...
			if (type === 1) {
				...
				try {
					const isProcessed =
						await bridgeContracts.subnet.admin.processedNonces(nonce);
					if (!isProcessed) {
						/* If not processed add tx to txs array */
					...
					console.log(
						"error while checking processedNonces on subnet bridge: ",
						error
					);
				}
			}
	...
	bridgeContracts.subnet.admin.on(
		...
		async (from, to, amount, date, nonce, type) => {
			/*
        		...
        		We only care for burn events as relayer. On burn events we will release on avax
			...
			if (type === 1) {
				...
				try {
					...
					console.log(
						"error while checking processedNonces on avax bridge: ",
						error
					);
				}
			}
